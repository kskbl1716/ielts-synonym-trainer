...
- 当前线上状态：**v11.1 已推送上线**（2026-08-11）：词库 **6141 词**、**10 本词书**、**难度分级**（lv）、**v11.0 智能记忆引擎**（掌握度5级/艾宾浩斯调度/今日待复习/错题本/例句朗读）+ **v11.1 记忆可视化**：统计页「🧠 智能记忆」复习看板（复习中/到期/未来7天/平均间隔/连续打卡 + 掌握度分布）与**遗忘曲线图**（inline SVG，账户平均预测留存曲线，按各词复习间隔 R=e^(-t/S)，含 60% 复习阈值虚线，站点首个 SVG 绘图）、「🔥 学习热力图」（12 周 GitHub 风格，数据源 `state.activity` 每日自动记录 + checkins/review.last 补历史）、每日活动记录 `state.activity`（云端按天取 max 合并）；v10.4 难度分级+三批扩词、v10.3 练习自动发音/首页更新、v10.2 SEO、v10.1 分页、v10.0 词书/背诵/路由均上线
