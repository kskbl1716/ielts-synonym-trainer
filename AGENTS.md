...
- 当前线上状态：**v11.6 体验完善批处理**（2026-08-12）：词库 **6533 词**、**13 本词书**、**难度分级**（lv）、**v11.0-11.5**（记忆引擎/可视化/词根词缀/手机布局/扩词/增强）+ **v11.6**（发音设置补全：语音检测+具体语音下拉+试听 `renderVoiceInfo`；闪卡正面去中文；新手引导 `maybeShowOnboard`；今日学习报告 `renderReportPanel`（dayStats 按日正确率）；搜索强化 d/note/mn/pos；导出图片 `exportImage` + 打印 PDF `exportPDF`；导入修复懒加载键；云同步手动按钮+上次同步+visibilitychange；SEO：`<h1>` + 样式移 body 底 + WORDS 元组序列化 2.11MB→1.71MB）；v10.4 难度分级+三批扩词、v10.3 练习自动发音/首页更新、v10.2 SEO、v10.1 分页、v10.0 词书/背诵/路由均上线
