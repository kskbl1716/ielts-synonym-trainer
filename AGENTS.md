...
- 当前线上状态：**v11.0 已推送上线**（2026-08-11）：词库 **6141 词**、**10 本词书**、**单词难度分级**（lv）；**v11.0 智能记忆引擎**：掌握度 5 级（陌生/认识/模糊/掌握/熟练，从 `state.stats` 派生）、艾宾浩斯遗忘曲线复习调度（`state.review`，答对 1→3→7→15→30 天递增/答错重置+错题本）、首页「今日待复习」队列（闪卡复习到期词）、统计页「错题本」面板（重练错题）、例句整句朗读、云端同步 review/notebook 合并；v10.4 难度分级 + 三批扩词、v10.3 练习自动发音/首页更新内容、v10.2 SEO、v10.1 分页、v10.0 词书/背诵/路由均上线
