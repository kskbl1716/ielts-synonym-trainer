...
- 功能：每日目标、打卡系统、生词本、错题本、进度统计、数据导出/导入、深色模式、字号调节、词书系统（**10 本词书**）、词库分页渲染、单词难度分级筛选、**艾宾浩斯智能复习**（今日待复习队列 + 例句朗读 + 遗忘曲线可视化 + 学习热力图）
- 当前版本 **v11.1**（2026-08-11）：在 v11.0 基础上新增 **记忆可视化**——统计页「智能记忆」复习看板（复习中/到期/未来7天/平均间隔/连续打卡 + 掌握度分布）、**遗忘曲线图**（inline SVG 按你的复习间隔自动绘制）、**学习热力图**（12 周学习足迹）；v11.0 提供掌握度5级/艾宾浩斯调度/今日待复习/错题本/例句朗读；历史发布清单见 V9-RELEASE.md / V10-PLAN.md
