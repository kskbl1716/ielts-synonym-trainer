'use strict';
/* V10 M5-6 oxford 牛津基础批次2 校验
   对照：work/_archive/.m5-oxford-kN.json（切片清单）+ .existing-words.txt（当前基准）+ 已接入批次
   检查：字段齐全、s>=2、z 合法、t 标准主题、pos 合法（n./v./adj./adv./prep.）、k 逐字在 e、
        例句 6~16 词、b 含 oxford、词条与切片一一对应、批内/跨库去重
   用法：node work/validate-v10-oxford-k.js
   产出：work/_archive/.m5-oxford-k-final.txt */
const fs = require('fs');
...
const SLICES = ['k1','k2','k3','k4','k5','k6','k7'];
const all = [];
const bySlice = {};
for (const n of SLICES){
  const mod = require(path.join(root, 'v10-oxford-' + n + '.js'));
  const arr = mod['V10OX_' + n.toUpperCase()] || [];
  bySlice[n] = arr;
  all.push(...arr);
}
...
require(path.join(root, 'v10-awl-f1.js')).V10AWL.forEach(w => existing.add(w.w));
require(path.join(root, 'v10-band9-h1.js')).V10B9.forEach(w => existing.add(w.w));
for (const n of ['j1','j2','j3','j4','j5','j6','j7']){
  const mod = require(path.join(root, 'v10-oxford-' + n + '.js'));
  (mod['V10OX_' + n.toUpperCase()] || []).forEach(w => existing.add(w.w));
}
const seen = new Set();
const missing = [], kErr = [], badTopic = [], badPos = [], badZone = [], badBook = [], dups = [], longE = [];
for (const w of all){
  ...
  if (!Array.isArray(w.b) || !w.b.includes('oxford')){ badBook.push(w.w); continue; }
...
console.log('=== V10 M5-6 oxford 批次2 校验 ===');
...
console.log('e>16w:', longE.length, JSON.stringify(longE.slice(0,25)));
...
for (const n of SLICES){
  const want = JSON.parse(fs.readFileSync(path.join(root, '_archive', '.m5-oxford-' + n + '.json'), 'utf8'));
...
fs.writeFileSync(path.join(root, '_archive', '.m5-oxford-k-final.txt'), [...seen].join('\n'), 'utf8');
console.log('written _archive/.m5-oxford-k-final.txt with', seen.size, 'words');
