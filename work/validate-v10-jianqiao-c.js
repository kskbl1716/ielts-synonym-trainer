'use strict';
/* V10 M5-2 jianqiao 真题高频批次校验
   对照：work/_archive/.m5-jianqiao-cN.json（切片清单）+ .existing-words.txt（3095 基准）+ 已接入批次（listening）
   检查：字段齐全、s>=2、z 合法、t 标准主题、pos 合法（含 prep.）、k 逐字在 e、b 含 jianqiao、
        词条与切片一一对应、批内/跨库去重
   用法：node work/validate-v10-jianqiao-c.js
   产出：work/_archive/.m5-jianqiao-c-final.txt */
const fs = require('fs');
...
for (const w of all){
  ...
  if (!Array.isArray(w.b) || !w.b.includes('jianqiao')){ badBook.push(w.w); continue; }
...
console.log('=== V10 M5-2 jianqiao 校验 ===');
...
for (const n of SLICES){
  const want = JSON.parse(fs.readFileSync(path.join(root, '_archive', '.m5-jianqiao-' + n + '.json'), 'utf8')).map(o => o.word.toLowerCase());
...
fs.writeFileSync(path.join(root, '_archive', '.m5-jianqiao-c-final.txt'), [...seen].join('\n'), 'utf8');
console.log('written _archive/.m5-jianqiao-c-final.txt with', seen.size, 'words');
