'use strict';
/* V10 M5-6 oxford 牛津基础批次1 校验
   对照：work/_archive/.m5-oxford-jN.json（切片清单）+ .existing-words.txt（当前基准）+ 已接入批次
   检查：字段齐全、s>=2、z 合法、t 标准主题、pos 合法（n./v./adj./adv./prep.）、k 逐字在 e、
        例句 6~16 词、b 含 oxford、词条与切片一一对应、批内/跨库去重
   用法：node work/validate-v10-oxford-j.js
   产出：work/_archive/.m5-oxford-j-final.txt */
const fs = require('fs');
...
const SLICES = ['j1','j2','j3','j4','j5','j6','j7'];
const all = [];
const bySlice = {};
...
console.log('=== V10 M5-6 oxford 批次1 校验 ===');
...
fs.writeFileSync(path.join(root, '_archive', '.m5-oxford-j-final.txt'), [...seen].join('\n'), 'utf8');
console.log('written _archive/.m5-oxford-j-final.txt with', seen.size, 'words');
