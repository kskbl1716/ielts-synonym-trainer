'use strict';
/* V10 M2 词书系统截图（CDP 驱动本地 Chrome）
   用法：node work/shot-v10-m2.js */
const fs = require('fs');
const path = require('path');

const BASE = 'http://127.0.0.1:9223';
const OUT = path.join(__dirname, 'v10-m2-');
...
(async () => {
  ...
  await cdp.shot('01-shelf.png', "switchView('books'); document.getElementById('books-detail').classList.add('hidden'); document.getElementById('books-shelf').classList.remove('hidden'); true");
  await cdp.shot('02-detail.png', "(()=>{ openBookDetail('default'); document.getElementById('book-search').value=''; renderBookWordList && renderBookWordList(); return true; })()");
  await cdp.shot('03-learn-filter.png', "(()=>{ switchView('learn'); learnBook='all'; learnZone='all'; learnTopic='all'; renderLearn(); return true; })()");
  await cdp.shot('04-stats-book.png', "(()=>{ switchView('stats'); return true; })()");
  await cdp.shot('05-settings-book.png', "(()=>{ switchView('settings'); return true; })()");
  await cdp.shot('06-modal-tags.png', "(()=>{ switchView('learn'); openWordDetail('crucial'); return true; })()");
