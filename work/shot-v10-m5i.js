'use strict';
/* V10 M5-6 oxford 批次2 截图（CDP）
   用法：node work/shot-v10-m5i.js */
const fs = require('fs');
const path = require('path');
const BASE = 'http://127.0.0.1:9223';
const OUT = path.join(__dirname, 'v10-m5i-');
...
(async () => {
  ...
  await cdp.shot('01-oxford-detail.png', "(()=>{ switchView('books'); openBookDetail('oxford'); return true; })()");
  await cdp.shot('02-shelf-all.png', "(()=>{ switchView('books'); document.getElementById('books-detail').classList.add('hidden'); document.getElementById('books-shelf').classList.remove('hidden'); return true; })()");
  await cdp.shot('03-stats.png', "(()=>{ switchView('stats'); return true; })()");
