'use strict';
/* V10 M5-3 zhenjing 雅思真经批次专项验证（CDP）
   前置：本地服务器 8000 + Chrome 9223
   用法：node work/e2e-v10-m5d.js */
const fs = require('fs');
...
(async () => {
  ...
  await T('z1-total', "WORDS.length");
  await T('z2-zhenjing-count', "bookWords('zhenjing').length");
  await T('z3-learn-filter', "(()=>{ learnBook='zhenjing'; learnZone='all'; learnTopic='all'; renderLearn(); return {cards:document.querySelectorAll('#learn-list .word-card').length, first:document.querySelector('#learn-list .wc-word')?document.querySelector('#learn-list .wc-word').textContent:null}; })()");
  await T('z4-detail', "(()=>{ openBookDetail('zhenjing'); return {words:document.querySelectorAll('#book-word-list .book-word-item').length}; })()");
  await T('z5-detail-search', "(()=>{ const i=document.getElementById('book-search'); i.value='hydrosphere'; i.dispatchEvent(new Event('input',{bubbles:true})); return document.querySelectorAll('#book-word-list .book-word-item').length; })()");
  await T('z6-fixed-typo', "(()=>{ const ws=bookWords('zhenjing'); return {vegetation:ws.some(x=>x.w==='vegetation'), reptile:ws.some(x=>x.w==='reptile'), noTypo:!ws.some(x=>/vagetation|pretail|approac|compus/.test(x.w))}; })()");
  await T('z7-flash-pool', "(()=>{ flashBook='zhenjing'; flashZone='all'; flashTopic='all'; startFlash(); return flash.list.length; })()");
  await T('z8-recitation', "(()=>{ openBookDetail('zhenjing'); document.getElementById('book-recite-btn').click(); const s=document.querySelector('#book-recite .br-setup'); return s? s.textContent.replace(/\\s+/g,' ').includes('共 448 词') : false; })()");
  await T('z9-shelf', "(()=>{ switchView('books'); const card=[...document.querySelectorAll('#books-shelf .book-card')].find(c=>c.textContent.includes('雅思真经')); return card?card.textContent.replace(/\\s+/g,' ').slice(0,40):null; })()");
  await T('z10-no-errors', "JSON.stringify(window.__errs||[])");

  console.log('==== E2E V10-M5d zhenjing 专项 (' + results.length + ' 项) ====');
  let fail = 0;
