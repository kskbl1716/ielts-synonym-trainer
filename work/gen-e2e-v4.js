...
const tests = [];
function T(name, expr){ tests.push([name, expr]); }
...
exit $fail
`;
const lines = tests.map(([n, e]) => 'T ' + ps(n) + ' ' + ps(e));
fs.writeFileSync('work/e2e-v4.ps1', harness + lines.join('\n') + '\n' + summary, 'utf8');
console.log('generated e2e-v4.ps1 with', tests.length, 'tests');