'use strict';
/* V10 M8 口语话题词校验：v10-spoken-add.js 32 个新增词条
   对照：outputs/index.html 已构建词库（去重）+ SPOKEN_WORDS 清单
   检查：字段齐全、s>=2、z 合法、t 标准主题、pos 合法、k 逐字在 e、b 含 spoken、词在清单内、去重
   用法：node work/validate-v10-spoken.js */
'use strict';
const fs = require('fs');
...
const { V10SPOKEN_ADD } = require(path.join(root, 'v10-spoken-add.js'));
const { SPOKEN_WORDS } = require(path.join(root, 'v10-spoken-words.js'));
const spokenSet = new Set(SPOKEN_WORDS.map(w=>String(w).toLowerCase()));
const seen = new Set();
const missing = [], kErr = [], badTopic = [], badPos = [], badZone = [], badBook = [], notSpoken = [], dups = [];
for (const w of V10SPOKEN_ADD){
  ...
  if (!Array.isArray(w.b) || !w.b.includes('spoken')){ badBook.push(w.w); continue; }
  if (!w.e.toLowerCase().includes(w.k.toLowerCase())){ kErr.push(w.w); continue; }
  if (!spokenSet.has(String(w.w).toLowerCase())){ notSpoken.push(w.w); continue; }
  if (site.has(String(w.w).toLowerCase())){ dups.push(w.w+' (in site)'); continue; }
  if (seen.has(w.w)){ dups.push(w.w+' (in-batch)'); continue; }
  seen.add(w.w);
}
console.log('=== V10 M8 口语话题词校验 ===');
console.log('total entries:', V10SPOKEN_ADD.length, '| OK:', seen.size);
...
console.log('not in SPOKEN list:', notSpoken.length, JSON.stringify(notSpoken));
