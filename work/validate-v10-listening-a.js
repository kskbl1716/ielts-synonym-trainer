'use strict';
/* V10 M5-1a listening 听力拼写批次校验
   对照：work/.m5-listening-aN.txt（切片清单）+ .existing-words.txt（去重）
   检查：字段齐全、s>=2、z='l'、t 标准主题、pos 合法、k 逐字在 e、b 含 listening、
        词条与切片清单一一对应、批内/跨库去重
   用法：node work/validate-v10-listening-a.js
   产出：work/.m5-listening-a-final.txt */
const fs = require('fs');
...
const SLICES = ['a1','a2','a3','a4','a5','a6'];
const all = [];
const bySlice = {};
...
for (const w of all){
  ...
  if (existing.has(w.w)){ dups.push(w.w+' (existing)'); continue; }
  if (seen.has(w.w)){ dups.push(w.w+' (in-batch)'); continue; }
  seen.add(w.w);
}
console.log('=== V10 M5-1a listening 校验 ===');
...
for (const n of SLICES){
  const want = fs.readFileSync(path.join(root, '.m5-listening-' + n + '.txt'), 'utf8').split(/\r?\n/).filter(Boolean).map(l => l.split('|')[0]);
...
fs.writeFileSync(path.join(root, '.m5-listening-a-final.txt'), [...seen].join('\n'), 'utf8');
console.log('written .m5-listening-a-final.txt with', seen.size, 'words');
