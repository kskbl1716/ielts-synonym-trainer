'use strict';
/* V10 M3 背诵模式截图（CDP 驱动本地 Chrome）
   用法：node work/shot-v10-m3.js */
const fs = require('fs');
const path = require('path');
const BASE = 'http://127.0.0.1:9223';
const OUT = path.join(__dirname, 'v10-m3-');
...
(async () => {
  ...
  await cdp.shot('01-recite-setup.png', "(()=>{ switchView('books'); openBookDetail('default'); document.getElementById('book-recite-btn').click(); return true; })()");
  await cdp.shot('02-recite-game.png', "(()=>{ beginReciteRound(WORDS.filter(w=>w.b.includes('default')).slice(0,3)); return true; })()");
  await cdp.shot('03-recite-repeat.png', "(()=>{ if(!recite.pool.length||recite.idx>=recite.pool.length) beginReciteRound(WORDS.filter(w=>w.b.includes('default')).slice(0,2)); document.getElementById('br-no').click(); return true; })()");
  await cdp.shot('04-recite-result.png', "(()=>{ while(!document.querySelector('#book-recite .br-result')){ const y=document.getElementById('br-yes'); const n=document.getElementById('br-no'); if(!y) break; y.click(); if(recite.wrong>=0 && document.querySelector('#book-recite .br-result')) break; } return true; })()");
