'use strict';
/* V10 M5-3 zhenjing 雅思真经批次校验
   对照：work/_archive/.m5-zhenjing-dN.json（切片清单）+ .existing-words.txt（3545 基准）+ 已接入批次
   检查：字段齐全、s>=2、z 合法、t 标准主题、pos 合法（含 prep.）、k 逐字在 e、b 含 zhenjing、
        词条与切片一一对应、批内/跨库去重
   用法：node work/validate-v10-zhenjing-d.js
   产出：work/_archive/.m5-zhenjing-d-final.txt */
const fs = require('fs');
...
const SLICES = ['d1','d2','d3','d4','d5','d6'];
const all = [];
const bySlice = {};
...
/* 已接入批次（listening a/b + jianqiao c）加入去重集 */
...
console.log('=== V10 M5-3 zhenjing 校验 ===');
...
fs.writeFileSync(path.join(root, '_archive', '.m5-zhenjing-d-final.txt'), [...seen].join('\n'), 'utf8');
console.log('written _archive/.m5-zhenjing-d-final.txt with', seen.size, 'words');
